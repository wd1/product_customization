		...
		function bike_load() {
			...
			fabric.Image.fromURL('images/'+val+'/'+val1+'.png', function(img) {
				...
				fabric.Image.fromURL('images/'+val+'/'+val1+'-Flywheel.png', function(img) {
					...
					canvas.add(wheel_img);
					wheel_img1 = img1.set({ left: mockup_img.width*mockup_img.scaleX+mockup_img.left-150, top: (this_wheel_position.y)*mockup_img.scaleY+mockup_img.top-180, scaleX:wheel_x_rate, scaleY:wheel_y_rate, angle: 0, selectable:true});
					if(!hide_wheelflag)
